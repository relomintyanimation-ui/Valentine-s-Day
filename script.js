...
// ============================================================
// REGION MAINTENANCE TOGGLE
// Set `active: true`  → region is LIVE and functional
// Set `active: false` → region shows MAINTENANCE message
// ============================================================
const REGION_CONFIG = {
    ind: { label: 'IND', active: true },  // ✅ LIVE
    na: { label: 'NA', active: true },  // ✅ LIVE
    bd: { label: 'BD', active: true },  // ✅ LIVE (existing)
    sg: { label: 'SG', active: true },  // ✅ LIVE (existing)
    pk: { label: 'PK', active: true },  // ✅ LIVE (existing)
    br: { label: 'BR', active: false },  // 🔧 MAINTENANCE
    us: { label: 'US', active: false },  // 🔧 MAINTENANCE
    sa: { label: 'SA', active: false },  // 🔧 MAINTENANCE
    mea: { label: 'MEA', active: false },  // 🔧 MAINTENANCE
    europe: { label: 'EUROPE', active: false },  // 🔧 MAINTENANCE
    id: { label: 'ID', active: false },  // 🔧 MAINTENANCE
};

const DISCORD_LINK = "https://discord.gg/codespecters"; // ← Change this to your Discord invite link
...
function applyRegionStyles() {
    document.querySelectorAll('.region-btn').forEach(btn => {
        ...
        const config = REGION_CONFIG[value];
        if (config && !config.active) {
            btn.classList.add('region-maintenance');
            btn.setAttribute('data-tooltip', `${config.label} — Under Maintenance\nJoin Discord for updates`);
        }
    });
}
...
/* Maintenance Check */
function checkRegionMaintenance(region) {
    const config = REGION_CONFIG[region ? region.toLowerCase() : ''];
    if (!config || !config.active) {
        const label = config ? config.label : (region || 'this region').toUpperCase();
        showMaintenanceToast(label);
        return true; // blocked
    }
    return false; // OK to proceed
}

function showMaintenanceToast(regionLabel) {
    const container = document.getElementById('notification-area');
    const toast = document.createElement('div');
    toast.className = 'toast maintenance';
    toast.innerHTML = `
        <i class="fa-solid fa-wrench"></i>
        <div>
            <strong>${regionLabel} Region — Under Maintenance</strong><br>
            <span style="font-size:0.85rem;">This region is not available yet. Get notified on
            <a href="${DISCORD_LINK}" target="_blank" style="color:#7289da;font-weight:700;">Discord</a>.</span>
        </div>
    `;
...
async function performEmote(id, name) {
    ...
    if (checkRegionMaintenance(performConfig.region)) return;
...
async function apiBotInvite() {
    ...
    if (checkRegionMaintenance(region)) return;
